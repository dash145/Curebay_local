...
function Slot(props) {
    ...
    const changeWeek = (val) => {
        if (val === "forward") {
            var newDate = moment(days[3], "DD/MM/yyyy")
                .add(1, "days")
                .format("DD/MM/yyyy");
    ...
    return (
        <div className="mt-1 -ml-20">
            <div className="flex">
                <div onClick={() => setAppointmentType("0")}
                    className={
                        appointmentType === "0"
            ...
            <div className="my-6 flex lg:flex-wrap  ">
                <button
                    ...
                    type="button"
                    className="disabled:opacity-50  rounded-lg w-20 h-12 mr-3"
                >
                    <i
                ...
                {days.map((res, i) => (
                    <button
                        ...
                        className={`${selectedDate === res
                            ...
                            } rounded-lg border w-28 h-12 p-2 mr-2`}
                    >
                        <p
                        ...
                        <p
                            className={` ${selectedDate !== res ? "text-brand-biscay2" : "text-white"
                                } font-medium text-sm -ml-1 w-14`}
                        >
                            {moment(res, "DD/MM/yyyy").format("DD, MMM")}
                        </p>
                    </button>
                ))}
                <button onClick={() => changeWeek("forward")} type="button" className="rounded-lg w-20 h-12 ml-2">
            ...
            <div className="w-96   mb-2">
                {slots.map((res, i) => (
                    <button onClick={() => { OnSelect(res.time); }} type="button" className={`border p-2 m-2 h-10 w-40 rounded text-xs font-normal ${selectedSlot === res.time ? "text-white bg-green-500" : "text-brand-primary"}`} >
                        {res.text}
                    </button>
                ))}
            </div>
